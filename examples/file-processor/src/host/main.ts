...
import { createApp } from './operations.ts';
...
Usage: ${APP_NAME} [options]
  ...
  --root <path>    Folder this application may read and write. Default: a
                   "workspace" folder inside the data directory below.
...
async function main(): Promise<number> {
  ...
  // The authorized root is decided *here*, by the operator, before any browser
  // is involved. The browser can name files inside it and nothing else.
  const rootFlag = argv.indexOf('--root');
  const explicit = rootFlag >= 0;
  if (explicit && argv[rootFlag + 1] === undefined) {
    console.error('--root needs a path');
    return 2;
  }
  const rootPath = explicit ? resolve(argv[rootFlag + 1] as string) : join(dataDir, 'workspace');
  await mkdir(rootPath, { recursive: true });

  const workspace: Workspace = {
    // Resolved through symlinks once, at startup, so every later containment
    // check compares against a canonical path.
    root: await realpath(rootPath),
    explicit,
    outputDirName: 'reports',
  };
