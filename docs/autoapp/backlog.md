...
| **A run timeout, or a no-progress limit, on an engineer's turn.** End a turn that has made no tool call for a set time, and say so, instead of letting a model plan in silence until the person gives up. | In 08c a turn spent twenty minutes emitting nothing after its last edit. A limit set before knowing whether guidance fixes that would cut off turns that were about to succeed. | 12d's measurement shows the stall persists with guidance. |
| **A `remove` command.** Deleting an application: its releases, its source workspace, its data and its journal rows. | Nothing here has ever deleted an application, because until now every one of them was something a person had deliberately imported. Creation makes them cheap, and cheap things accumulate. | Design it with `prune`, above, and the same rules: list what would go, require `--yes`, refuse while the application is serving, and never remove data without saying that is what is being removed. |
